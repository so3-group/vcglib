...
void AmbiVector<_Scalar,_Index>::setZero()
{
  ...
  {
    eigen_vcg_assert(m_mode==IsSparse);
    m_llSize = 0;
    m_llStart = -1;
  }
}
...
_Scalar& AmbiVector<_Scalar,_Index>::coeffRef(_Index i)
{
  ...
  else
  {
    ...
    else
    {
      ...
      eigen_vcg_assert(i>=llElements[m_llCurrent].index && "you must call restart() before inserting an element with lower or equal index");
      ...
      else
      {
        ...
        eigen_internal_vcg_assert(m_llSize<m_allocatedElements && "internal error: overflow in sparse mode");
