  ...
  : public SparseMatrixBase<SparseMatrix<_Scalar, _Options, _Index> >
{
  ...
  public:
    ...
    {
      eigen_vcg_assert(row>=0 && row<rows() && col>=0 && col<cols());
  ...
  public:
    ...
    {
      eigen_vcg_assert(isCompressed() && "This function does not make sense in non compressed mode.");
  ...
  public:
    ...
    {
      eigen_vcg_assert(size_t(m_outerIndex[outer+1]) == m_data.size() && "Invalid ordered insertion (invalid outer index)");
      eigen_vcg_assert( (m_outerIndex[outer+1]-m_outerIndex[outer]==0 || m_data.index(m_data.size()-1)<inner) && "Invalid ordered insertion (invalid inner index)");
    ...
    {
      eigen_vcg_assert(m_outerIndex[outer]==Index(m_data.size()) && "You must call startVec for each inner vector sequentially");
      eigen_vcg_assert(m_outerIndex[outer+1]==0 && "You must call startVec for each inner vector sequentially");
    ...
    {
      eigen_vcg_assert(rows() == cols() && "ONLY FOR SQUARED MATRICES");
...
public:
    ...
    EIGEN_STRONG_INLINE Scalar& insertBackUncompressed(Index row, Index col)
    {
      ...
      eigen_vcg_assert(!isCompressed());
      eigen_vcg_assert(m_innerNonZeros[outer]<=(m_outerIndex[outer+1] - m_outerIndex[outer]));
...
void set_from_triplets(const InputIterator& begin, const InputIterator& end, SparseMatrixType& mat, int Options = 0)
{
  ...
  if(begin!=end)
  {
    ...
    {
      eigen_vcg_assert(it->row()>=0 && it->row()<mat.rows() && it->col()>=0 && it->col()<mat.cols());
...
EIGEN_DONT_INLINE typename SparseMatrix<_Scalar,_Options,_Index>::Scalar& SparseMatrix<_Scalar,_Options,_Index>::insertUncompressed(Index row, Index col)
{
  ...
  eigen_vcg_assert((p<=startId || m_data.index(p-1)!=inner) && "you cannot insert an element that already exist, you must call coeffRef to this end");
