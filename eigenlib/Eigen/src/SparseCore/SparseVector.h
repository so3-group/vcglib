  ...
  : public SparseMatrixBase<SparseVector<_Scalar, _Options, _Index> >
{
  ...
  public:
    ...
    {
      eigen_vcg_assert(IsColVector ? (col==0 && row>=0 && row<m_size) : (row==0 && col>=0 && col<m_size));
      return coeff(IsColVector ? row : col);
    }
    inline Scalar coeff(Index i) const
    {
      eigen_vcg_assert(i>=0 && i<m_size);
      return m_data.at(i);
    }
  ...
  public:
    ...
    {
      eigen_vcg_assert(rows==1 || cols==1);
...
struct sparse_vector_assign_selector<Dest,Src,SVA_Inner> {
  static void run(Dest& dst, const Src& src) {
    eigen_internal_vcg_assert(src.innerSize()==src.size());
...
struct sparse_vector_assign_selector<Dest,Src,SVA_Outer> {
  static void run(Dest& dst, const Src& src) {
    eigen_internal_vcg_assert(src.outerSize()==src.size());
