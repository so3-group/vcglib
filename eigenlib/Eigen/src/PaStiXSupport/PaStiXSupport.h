...
class PastixBase : internal::noncopyable
{
  ...
  public:
    ...
    {
      eigen_vcg_assert(m_isInitialized && "Pastix solver is not initialized.");
    ...
    {
      eigen_vcg_assert(m_isInitialized && "Pastix LU, LLT or LDLT is not initialized.");
      eigen_vcg_assert(rows()==b.rows()
                && "PastixBase::solve(): invalid number of rows of the right hand side matrix b");
  ...
  protected:
    ...
    void clean()
    {
      eigen_vcg_assert(m_initisOk && "The Pastix structure should be allocated first"); 
...
{
  eigen_vcg_assert(mat.rows() == mat.cols() && "The input matrix should be squared");
...
{                         
  eigen_vcg_assert(m_initisOk && "The initialization of PaSTiX failed");
...
//   if(&m_cpyMat != &mat) m_cpyMat = mat;
  eigen_vcg_assert(m_analysisIsOk && "The analysis phase should be called before the factorization phase");
...
{
  eigen_vcg_assert(m_isInitialized && "The matrix should be factorized first");
