...
class IncompleteLUT : internal::noncopyable
{
  ...
  public:
    ...
    {
      eigen_vcg_assert(m_isInitialized && "IncompleteLUT is not initialized.");
      return m_info;
    }
...
void IncompleteLUT<Scalar>::factorize(const _MatrixType& amat)
{
  ...
  eigen_vcg_assert((amat.rows() == amat.cols()) && "The factorization should be done on a square matrix");
  ...
  for (Index ii = 0; ii < n; ii++)
  {
    ...
    while (jj < sizel)
    {
      ...
      eigen_internal_vcg_assert(ki_it && ki_it.col()==minrow);
      ...
      for (; ki_it; ++ki_it)
      {
        ...
        if (jpos == -1) // fill-in element
        {
          ...
          if (j >= ii) // dealing with the upper part
          {
            newpos = ii + sizeu;
            sizeu++;
            eigen_internal_vcg_assert(sizeu<=n);
          }
          else // dealing with the lower part
          {
            newpos = sizel;
            sizel++;
            eigen_internal_vcg_assert(sizel<=ii);
          }
          ju(newpos) = j;
          u(newpos) = -prod;
          jr(j) = newpos;
