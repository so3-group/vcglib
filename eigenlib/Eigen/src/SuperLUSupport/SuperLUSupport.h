...
struct SluMatrix : SuperMatrix
{
  ...
  void setStorageType(Stype_t t)
  {
    ...
    {
      eigen_vcg_assert(false && "storage type not supported");
      Store = 0;
    }
  }
  ...
  void setScalarType()
  {
    ...
    {
      eigen_vcg_assert(false && "Scalar type not supported by SuperLU");
    }
  }
  ...
  static SluMatrix Map(MatrixBase<MatrixType>& _mat)
  {
    ...
    eigen_vcg_assert( ((MatrixType::Flags&RowMajorBit)!=RowMajorBit) && "row-major dense matrices are not supported by SuperLU");
    SluMatrix res;
  ...
  static SluMatrix Map(SparseMatrixBase<MatrixType>& mat)
  {
    ...
    eigen_vcg_assert(((MatrixType::Flags & SelfAdjoint)==0) && "SelfAdjoint matrix shape not supported by SuperLU");
...
struct SluMatrixMapHelper<Matrix<Scalar,Rows,Cols,Options,MRows,MCols> >
{
  ...
  {
    eigen_vcg_assert( ((Options&RowMajor)!=RowMajor) && "row-major dense matrices is not supported by SuperLU");
    res.setStorageType(SLU_DN);
...
{
  eigen_vcg_assert((Flags&RowMajor)==RowMajor && sluMat.Stype == SLU_NR
...
class SuperLUBase : internal::noncopyable
{
  ...
  public:
    ...
    {
      eigen_vcg_assert(m_isInitialized && "SuperLU is not initialized.");
...
{
  eigen_vcg_assert(m_analysisIsOk && "You must first call analyzePattern()");
  if(!m_analysisIsOk)
  {
    m_info = InvalidInput;
    return;
...
{
  eigen_vcg_assert(m_factorizationIsOk && "The decomposition is not in a valid state for solving, you must first call either compute() or analyzePattern()/factorize()");

  const int size = m_matrix.rows();
  const int rhsCols = b.cols();
  eigen_vcg_assert(size==b.rows());
...
{
  eigen_vcg_assert(m_factorizationIsOk && "The decomposition is not in a valid state for extracting factors, you must first call either compute() or analyzePattern()/factorize()");
...
{
  eigen_vcg_assert(m_factorizationIsOk && "The decomposition is not in a valid state for computing the determinant, you must first call either compute() or analyzePattern()/factorize()");
  ...
  for (int j=0; j<m_u.cols(); ++j)
  {
    if (m_u.outerIndexPtr()[j+1]-m_u.outerIndexPtr()[j] > 0)
    {
      ...
      eigen_vcg_assert(m_u.innerIndexPtr()[lastId]<=j);
