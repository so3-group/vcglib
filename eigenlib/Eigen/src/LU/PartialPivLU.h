...
template<typename _MatrixType> class PartialPivLU
{
  public:
    ...
    */
    ...
    {
      eigen_vcg_assert(m_isInitialized && "PartialPivLU is not initialized.");
      return m_lu;
    }
...
{
  eigen_vcg_assert(lu.cols() == row_transpositions.size());
  eigen_vcg_assert((&row_transpositions.coeffRef(1)-&row_transpositions.coeffRef(0)) == 1);
...
PartialPivLU<MatrixType>& PartialPivLU<MatrixType>::compute(const MatrixType& matrix)
{
  ...
  eigen_vcg_assert(matrix.rows()<NumTraits<int>::highest());
  
  m_lu = matrix;

  eigen_vcg_assert(matrix.rows() == matrix.cols() && "PartialPivLU is only for square (and moreover invertible) matrices");
  ...
  : solve_retval_base<PartialPivLU<_MatrixType>, Rhs>
{
  ...
  template<typename Dest> void evalTo(Dest& dst) const
  {
    ...
    */
    ...
    eigen_vcg_assert(rhs().rows() == dec().matrixLU().rows());
