  ...
  const DynMatrixType& C)
{
  ...
  eigen_vcg_assert(A.isUpperTriangular());
  eigen_vcg_assert(B.rows() == B.cols());
  eigen_vcg_assert(B.isUpperTriangular());
  eigen_vcg_assert(C.rows() == A.rows());
  eigen_vcg_assert(C.cols() == B.rows());
