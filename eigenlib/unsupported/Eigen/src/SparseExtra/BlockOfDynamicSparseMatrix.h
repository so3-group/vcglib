  ...
  : public SparseMatrixBase<SparseInnerVectorSet<DynamicSparseMatrix<_Scalar, _Options, _Index>, Size> >
{
  ...
  public:
    ...
    {
      eigen_vcg_assert( (outerStart>=0) && ((outerStart+outerSize)<=matrix.outerSize()) );
    }

    inline SparseInnerVectorSet(const MatrixType& matrix, Index outer)
      : m_matrix(matrix), m_outerStart(outer), m_outerSize(Size)
    {
      eigen_vcg_assert(Size!=Dynamic);
      eigen_vcg_assert( (outer>=0) && (outer<matrix.outerSize()) );
    }
    ...
    const Scalar& lastCoeff() const
    {
      ...
      eigen_vcg_assert(m_matrix.data()[m_outerStart].size()>0);
