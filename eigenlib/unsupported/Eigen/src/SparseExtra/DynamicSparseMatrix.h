  ...
  : public SparseMatrixBase<DynamicSparseMatrix<_Scalar, _Options, _Index> >
{
  ...
  public:
    ...
    {
      eigen_vcg_assert(outer<Index(m_data.size()) && inner<m_innerSize && "out of range");
      eigen_vcg_assert(((m_data[outer].size()==0) || (m_data[outer].index(m_data[outer].size()-1)<inner))
    ...
    {
      eigen_vcg_assert(innerSize()==0 && outerSize()==0);
    }
