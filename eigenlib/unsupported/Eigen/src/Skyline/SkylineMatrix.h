...
public:
    ...
    inline Scalar coeff(Index row, Index col) const {
        ...
        eigen_vcg_assert(outer < outerSize());
        eigen_vcg_assert(inner < innerSize());
    ...
    inline Scalar& coeffRef(Index row, Index col) {
        ...
        if (IsRowMajor) {
            ...
            {
                const Index minOuterIndex = inner - m_data.upperProfile(inner);
                eigen_vcg_assert(outer >= minOuterIndex && "you try to acces a coeff that do not exist in the storage");
                return this->m_data.upper(m_colStartIndex[inner] + outer - (inner - m_data.upperProfile(inner)));
            }
            if (col < row) //lower matrix
            {
                const Index minInnerIndex = outer - m_data.lowerProfile(outer);
                eigen_vcg_assert(inner >= minInnerIndex && "you try to acces a coeff that do not exist in the storage");
                return this->m_data.lower(m_rowStartIndex[outer] + inner - (outer - m_data.lowerProfile(outer)));
            }
        } else {
            if (outer > inner) //upper matrix
            {
                const Index maxOuterIndex = inner + m_data.upperProfile(inner);
                eigen_vcg_assert(outer <= maxOuterIndex && "you try to acces a coeff that do not exist in the storage");
                return this->m_data.upper(m_colStartIndex[inner] + (outer - inner));
            }
            if (outer < inner) //lower matrix
            {
                const Index maxInnerIndex = outer + m_data.lowerProfile(outer);
                eigen_vcg_assert(inner <= maxInnerIndex && "you try to acces a coeff that do not exist in the storage");
                return this->m_data.lower(m_rowStartIndex[outer] + (inner - outer));
            }
        }
    }
    ...
    inline Scalar coeffDiag(Index idx) const {
        eigen_vcg_assert(idx < outerSize());
        eigen_vcg_assert(idx < innerSize());
...
public:
    ...
    inline void finalize() {
        if (IsRowMajor) {
            ...
            //            eigen_vcg_assert(rows() == cols() && "memory reorganisatrion only works with suare matrix");
            //
    ...
    void resize(size_t rows, size_t cols) {
        ...
        eigen_vcg_assert(rows == cols && "Skyline matrix must be square matrix");
